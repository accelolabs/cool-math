...
**Односвязный список** - связный список, в котором в узлах хранятся данные и указатель на следующий элемент в списке. <br/>
...
<!-- Вывод максимума - смотри в макстек -->
...
Чтобы найти эту $node$ требуется запустить обычный алгоритм зайца и черепахи. Так мы можем удостовериться, что цикл есть, и найти дальнейшую позицию для запуска. Далее мы запускаем вторую черепашку с $node$ со значение 3. Теперь будем итерироваться зайцем и черепахой по одной $node$, пока они не встретятся. Гарантируется, что они встретятся в $node$ , которая является началом цикла.
